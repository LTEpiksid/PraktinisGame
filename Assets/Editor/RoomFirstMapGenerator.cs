...
public class RoomFirstMapGenerator : SimpleRandomWalkMapGenerator
{
    ...
    [SerializeField]
    private Tilemap wallTilemap; // Reference to the tilemap for wall tiles

    [SerializeField]
    private TileBase floorTile; // Placeholder for the floor tile
    [SerializeField]
    private TileBase wallTile; // Placeholder for the wall tile
    ...
    private void PaintFloorTiles(HashSet<Vector2Int> floor)
    {
    ...
    private void CreateWalls(HashSet<Vector2Int> floor)
    {
        foreach (var position in floor)
        {
            for (int x = position.x - 1; x <= position.x + 1; x++)
            {
                for (int y = position.y - 1; y <= position.y + 1; y++)
                {
                    ...
                    {
                        wallTilemap.SetTile((Vector3Int)new Vector2Int(x, y), wallTile);
                    }
                }
            }
        }
